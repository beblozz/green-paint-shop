...
function CareFiltersModal({ isOpen, onClose, activeFilters, onApply }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content-sidebar" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="filter-group">
          ...
          {['Все товары', 'Лейки', 'Опрыскиватели', 'Секаторы', 'Перчатки', 'Инструменты'].map((cat, i) => (
        ...
        <div className="filter-group">
          ...
          {['Полив', 'Пересадка', 'Обрезка'].map((purp, i) => (
            <label key={i} className="filter-checkbox-label">
              <div className="checkbox-wrapper">
                <input 
                  type="checkbox" 
                  className="custom-checkbox" 
                  checked={purposes.includes(purp)}
                  onChange={() => handleCheckboxChange(purposes, setPurposes, purp)}
                />
                {purposes.includes(purp) && <img src="/images/Done.png" alt="checked" className="done-icon" />}
              </div>
              <span>{purp}</span>
            </label>
          ))}
        ...
        <div className="filter-group">
          ...
          {['Пластик', 'Металл', 'Стекло'].map((mat, i) => (
            <label key={i} className="filter-checkbox-label">
              <div className="checkbox-wrapper">
                <input 
                  type="checkbox" 
...
export default CareFiltersModal;